...
const addSkill = asyncHandler(async (req, res, next) => {
  ...
  // Case-insensitive EXACT match on the escaped name, so "C++", "C#",
  // ".NET", and "Node.js" all work correctly instead of throwing
  // "Invalid regular expression" (unescaped "+", "#", "." are regex
  // metacharacters) and instead of accidentally matching substrings.
  const existing = await Skill.findOne({
    user: req.user._id,
    skillName: new RegExp(`^${escapeRegExp(trimmedName)}$`, 'i'),
  });
