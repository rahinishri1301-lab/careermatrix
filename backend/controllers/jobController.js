...
const getAllJobs = asyncHandler(async (req, res) => {
  ...
  // Attach hasApplied so the Flutter Opportunity Center can show accurate
  // per-job "Applied" status instead of only discovering it when the
  // student taps Apply and gets a 400 back.
  const appliedJobIds = new Set(
    (
      await JobApplication.find({ applicant: req.user._id, job: { $in: jobs.map((j) => j._id) } }).distinct('job')
    ).map(String)
  );
  const jobsWithAppliedFlag = jobsWithMatch.map((j) => ({
    ...j,
    hasApplied: appliedJobIds.has(String(j._id)),
  }));

  return sendResponse(res, 200, true, 'Jobs fetched successfully', jobsWithAppliedFlag, {
