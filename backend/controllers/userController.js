...
/**
 * @desc    Browse alumni (with their Profile info) so students can discover
 *          and request mentorship from them. Open to any authenticated
 *          user (unlike getCandidates, which is restricted) since browsing
 *          alumni is a core Student-module feature.
 * @route   GET /api/users/alumni?search=jane&page=1&limit=20
 * @access  Private
 */
const getAlumni = asyncHandler(async (req, res) => {
  ...
  const userFilter = { role: 'alumni', isActive: { $ne: false } };
  if (req.query.search) {
    const searchVal = escapeRegExp(req.query.search);
    userFilter.$or = [
      { name: { $regex: searchVal, $options: 'i' } },
  ...
  return sendResponse(res, 200, true, 'Alumni fetched successfully', alumni, {
    pagination: { total, page, pages: Math.ceil(total / limit), limit },
  });
});
