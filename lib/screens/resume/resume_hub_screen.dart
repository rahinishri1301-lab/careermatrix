...
class _ResumeHubScreenState extends State<ResumeHubScreen> {
  ...
  void _viewResume() {
    if (kIsWeb) {
      BackendRepository.instance.viewResumeInNewTab();
    } else {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => ResumePreviewScreen(fileName: _fileName),
        ),
      );
    }
  }
  ...
  Future<void> _download() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(kIsWeb ? 'Downloaded ${_fileName ?? path}' : 'Saved to $path'),
                          ...
                          if (_hasResume) ...[
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                onPressed: _busy ? null : _viewResume,
                            ...
                            Row(
                              children: [
                                ...
                                Expanded(
                                  child: OutlinedButton.icon(
                                    onPressed: _busy ? null : () => _pickAndUpload(replacing: true),
                            ...
                            SizedBox(
                              width: double.infinity,
                              child: ElevatedButton.icon(
                                onPressed: _busy ? null : () => _pickAndUpload(replacing: false),
                                icon: const Icon(Icons.upload_rounded, size: 18),
